...
The purpose of this page is to test and demonstrate an approach to
finding the front of a parcel. This is assuming we have a dataset with
the boundaries of all parcels in a continguous area if interest and
another dataset defining street centerlines.
...
The front of the parcel will be the side that's not shared with another
parcel and is closest to its closest parallel street. If the closest
street segment isn't parallel (within a threshold) I'm assuming the
second-closest is.

The rear will be parallel to the front, and everything that isn't a
front or a rear will be a side.
...
because the side of one parcel is being drawn on top of the rear of
another.
...
``` r
{r, message=FALSE, warning=FALSE}
st_write(edges_labeled_more, "edges_labeled_more.geojson") 
```
...
```{r, message=FALSE, results='hide'}
minn_parcels <- here("data",
                "Minn_parcels.gpkg") |>
  st_read()

minn_streets <- here("data",
                "PW_Street_Centerline.geojson") |>
  st_read()
```
...
```{r, message=FALSE, warning=FALSE}
edges_labeled_all <- label_sides(minn_parcels, minn_streets)

st_write(edges_labeled_all, "edges_labeled_all.geojson") 

side_pal <- colorFactor(palette = brewer.pal(4, "Dark2"),
                        domain = edges_labeled_all$side)
